...
1. Repo scaffolding
   ...
   - Success: extension can send/receive typed messages to app.
...
5. MLX 4-bit model packaging + Swift runtime
   ...
   - Implement a Swift XPC worker in `src/laika/model` that loads the MLX model via `mlx-swift` and exposes a typed inference API to the app.
