...
2) Waits a short post-load delay (configurable) and calls `window.LaikaHarness.observeDom()` to produce an Observation using the same extraction logic as the extension (handles, blocks, items, outline, comments).
   - If the observation is empty (no text/items/blocks/comments/elements), the harness waits briefly and retries once before continuing.
...
- `--debug-observe` to include `observe_dom` debug payloads (timings, root selection, counts) in the JSON output.
